#!/bin/bash
# Build and launch the BorderBeam showcase in the iOS Simulator.
#
...
BUNDLE_ID="com.jakubantalik.BorderBeamShowcase"
...
echo "▸ Building…"
xcodebuild \
  -project BorderBeamShowcase.xcodeproj \
  -scheme BorderBeamShowcase \
  -destination "platform=iOS Simulator,name=$DEVICE" \
  -derivedDataPath build \
  build > /tmp/borderbeam-showcase-build.log 2>&1 \
  || { grep -E "error:" /tmp/borderbeam-showcase-build.log | head -20; \
       echo "BUILD FAILED (full log: /tmp/borderbeam-showcase-build.log)"; exit 1; }

APP="build/Build/Products/Debug-iphonesimulator/BorderBeamShowcase.app"
